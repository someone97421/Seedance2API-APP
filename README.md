...
- 📂 **富媒体素材管理**：
  ...
  - 支持 **拖拽上传** 和 **URL 链接** 导入。
  - 自动将上传的本地图片借助 ImgBB 临时转换并托管为公开 URL（由于 API 要求）。
- ⚡ **多任务并发与轮询**：
...
- ⚙️ **完整的本地配置存储**：
  - 支持自定义 API Base URL、API Key、ImgBB Key。
...
2. **ImgBB 设置 (可选但推荐)**：获取一个免费的 [ImgBB API Key](https://api.imgbb.com/)，用于本地拖拽图片上传。
...
- 📂 **Rich Media Asset Management**:
  ...
  - Automatically hosts local image uploads via the ImgBB API (as the Seedance API requires public URLs for image references).
...
- ⚙️ **Persistent Local Storage**:
  - Customizable API Base URL, API Key, and ImgBB Key.
...
2. **ImgBB Setup (Optional but recommended)**: Obtain a free [ImgBB API Key](https://api.imgbb.com/) to enable local drag-and-drop image uploads.
