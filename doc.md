...
```mixin-react
return (<Tabs>
<Tabs.TabPane title="Windows 用户" key="wxuWlgLhfk"><RenderMd content={`1. 进入 \`scripts/init_dev_env\` 目录。
...
<Tabs.TabPane title="macOS 用户" key="QVzgsy5whY"><RenderMd content={`1. 打开终端，进入 \`scripts/init_dev_env\` 目录。
2. 运行构建脚本：
...
```mixin-react
return (<Tabs>
<Tabs.TabPane title="Python" key="IOWtx3ccAZ"><RenderMd content={`\`\`\`Python
import os
import time
...
```mixin-react
return (<Tabs>
<Tabs.TabPane title="Python" key="MvEZkxaIS3"><RenderMd content={`\`\`\`Python
import os
import time
# Install SDK:  pip install 'volcengine-python-sdk[ark]'
from volcenginesdkarkruntime import Ark 
...
```mixin-react
return (<Tabs>
<Tabs.TabPane title="Python" key="GuiGnQoLCQ"><RenderMd content={`\`\`\`Python
import os
import time
# Install SDK:  pip install 'volcengine-python-sdk[ark]'
from volcenginesdkarkruntime import Ark 
...
```mixin-react
return (<Tabs>
<Tabs.TabPane title="Curl" key="X6f1w2pAkL"><RenderMd content={`1. 创建视频生成任务
...
\`\`\`Shell
//请将 cgt-2026****hzc2z 替换为步骤一获得的视频生成任务ID
...
## 更多能力
Seedance 2.0 系列模型也支持文生视频、首帧图生视频、首尾帧图生视频、设置视频输出规格等通用基础能力，详情请参见 [视频生成](/docs/82379/1366799)。
...
| |> 虚拟人像| |\
| || |\
| |<span>![图片](https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/791b783fc6cd4394b13f41b66b5ff461~tplv-goo7wpa0wc-image.image =200x) </span>| |\
| |> 产品图像 | |

:::warning
\*\*Asset ID 仅用来向模型传入素材，不支持使用 Asset ID 指代素材。\*\*提示词中仍需使用"**素材类型+序号**”格式引用素材，序号为请求体中该素材在同类素材中的排序。例如 「图片 n」指代`content`数组中第 n 个`type="image_url"`的参考图片（按数组顺序从1开始计数）。
:::
```Python
import os
import time
...
|第一次输出视频 |二次编辑后视频 |
|---|---|
|<video src="https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/24e27818aeb644b6942c2cbc949ddc86~tplv-goo7wpa0wc-image.image" controls></video>|<video src="https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/402097387ab14cb79afa39f191fd6640~tplv-goo7wpa0wc-image.image" controls></video>|\
|||\
...
```mixin-react
return (<Tabs>
<Tabs.TabPane title="Python" key="A4aZxYSm11"><RenderMd content={`1. 首次生视频，并获取视频 URL。此处直接用[使用虚拟人像](/docs/82379/2277377#2bf01416)的例子。
...
下文介绍多模态参考、编辑视频、延长视频的提示词典型公式，更多详细内容请参见[Seedance 2.0 提示词指南](/docs/82379/2222480)。
:::tip
平台提供 **Seedance 2.0 提示词优化技能**，方便您对提示词进行调优。

* 配置方式：可将技能文件配置到任意 Code Agent 中使用。以 Trae 为例，配置步骤如下：打开 **设置** 页面，选择 **规则与技能**，点击 **创建技能** 并上传该文件，技能名称设为 `sd2-pe`。上传完成后路径为：`.trae/skills/sd2-pe/SKILL.md`。
* 使用方式：在 AI 输入框输入 `/sd2-pe + 自定义提示词`，即可开始调试提示词。
